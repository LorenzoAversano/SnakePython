...
def print_score (score):
    text = scr_font.render ("Score: "+str(score), True, white)
...
def game_start():
    ...
    snake_pixels = [] 
    snake_length = 1
    target_x, target_y = round(random.randrange(0,width-snake_size)/10.0)*10.0, round(random.randrange(0,height-snake_size)/10.0)*10.0
    while not game_over:
        while game_close:
            screen.blit(bg_image, [0, 0])
            game_over_message = msg_font.render("Game Over! Press A to Play Again", True, red)
            ...
            high_score_message = msg_font.render("High Score: "+str(high_score), True, white)
